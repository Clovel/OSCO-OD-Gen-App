 ...
 * @memberof Eds
 */
const rawToType = (pRawData: Buffer, pDataType: DataType) => {
  let lOutput: string = '';
  switch(pDataType) {
    case DataType.BOOLEAN:
      return !!pRawData.readUInt8();
    case DataType.INTEGER8:
      return pRawData.readInt8();
    case DataType.INTEGER16:
      return pRawData.readInt16LE();
    case DataType.INTEGER24:
      return pRawData.readIntLE(0, 3);
    case DataType.INTEGER32:
      return pRawData.readInt32LE();
    case DataType.INTEGER40:
      return pRawData.readIntLE(0, 5);
    case DataType.INTEGER48:
      return pRawData.readIntLE(0, 6);
    case DataType.INTEGER64:
      return pRawData.readBigInt64LE();
    case DataType.UNSIGNED8:
      return pRawData.readUInt8();
    case DataType.UNSIGNED16:
      return pRawData.readUInt16LE();
    case DataType.UNSIGNED24:
      return pRawData.readUIntLE(0, 3);
    case DataType.UNSIGNED32:
      return pRawData.readUInt32LE();
    case DataType.UNSIGNED40:
      return pRawData.readUIntLE(0, 5);
    case DataType.UNSIGNED48:
      return pRawData.readUIntLE(0, 6);
    case DataType.UNSIGNED64:
      return pRawData.readBigUInt64LE();
    case DataType.REAL32:
      return pRawData.readFloatLE();
    case DataType.REAL64:
      return pRawData.readDoubleLE();
    case DataType.VISIBLE_STRING:
    case DataType.UNICODE_STRING:
      lOutput = pRawData.toString();
      const end = lOutput.indexOf('\0');

      if(end !== -1) {
        lOutput = lOutput.substring(0, end);
      }
      return lOutput;
    case DataType.TIME_OF_DAY:
    case DataType.TIME_DIFFERENCE:
      const ms: number = pRawData.readUInt32LE(0);
      const days: number = pRawData.readUInt16LE(4);
 ...
 * @memberof Eds
 */
const typeToRaw = (pValue: number | bigint | string | Date, pDataType : DataType) => {
  // eslint-disable-next-line init-declarations
  let lRawData: Buffer;
  switch(pDataType) {
    case DataType.BOOLEAN:
      // eslint-disable-next-line @typescript-eslint/strict-boolean-expressions
      lRawData = Buffer.from(pValue ? [1] : [0]);
      break;
    case DataType.INTEGER8:
      lRawData = Buffer.alloc(1);
      lRawData.writeInt8(pValue as number);
      break;
    case DataType.UNSIGNED8:
      lRawData = Buffer.alloc(1);
      lRawData.writeUInt8(pValue as number);
      break;
    case DataType.INTEGER16:
      lRawData = Buffer.alloc(2);
      lRawData.writeInt16LE(pValue as number);
      break;
    case DataType.UNSIGNED16:
      lRawData = Buffer.alloc(2);
      lRawData.writeUInt16LE(pValue as number);
      break;
    case DataType.INTEGER24:
      lRawData = Buffer.alloc(3);
      lRawData.writeIntLE(pValue as number, 0, 3);
      break;
    case DataType.UNSIGNED24:
      lRawData = Buffer.alloc(3);
      lRawData.writeUIntLE(pValue as number, 0, 3);
      break;
    case DataType.INTEGER32:
      lRawData = Buffer.alloc(4);
      lRawData.writeInt32LE(pValue as number);
      break;
    case DataType.UNSIGNED32:
      lRawData = Buffer.alloc(4);
      lRawData.writeUInt32LE(pValue as number);
      break;
    case DataType.INTEGER40:
      lRawData = Buffer.alloc(5);
      lRawData.writeIntLE(pValue as number, 0, 5);
      break;
    case DataType.UNSIGNED40:
      lRawData = Buffer.alloc(5);
      lRawData.writeUIntLE(pValue as number, 0, 5);
      break;
    case DataType.INTEGER48:
      lRawData = Buffer.alloc(6);
      lRawData.writeIntLE(pValue as number, 0, 6);
      break;
    case DataType.UNSIGNED48:
      lRawData = Buffer.alloc(6);
      lRawData.writeUIntLE(pValue as number, 0, 6);
      break;
    case DataType.INTEGER64:
      if(typeof pValue !== 'bigint') {
        pValue = BigInt(pValue);
      }

      lRawData = Buffer.alloc(8);
      lRawData.writeBigInt64LE(pValue);
      break;
    case DataType.UNSIGNED64:
      if(typeof pValue !== 'bigint') {
        pValue = BigInt(pValue);
      ...
      lRawData = Buffer.alloc(8);
      lRawData.writeBigUInt64LE(pValue);
      break;
    case DataType.REAL32:
      lRawData = Buffer.alloc(4);
      lRawData.writeFloatLE(pValue as number);
      break;
    case DataType.REAL64:
      lRawData = Buffer.alloc(8);
      lRawData.writeDoubleLE(pValue as number);
      break;
    case DataType.VISIBLE_STRING:
      lRawData = (pValue) ? Buffer.from(pValue) : Buffer.alloc(0);
      const end = lRawData.indexOf('\0');
      if(end !== -1) { lRawData = lRawData.subarray(0, end); }
      break;
    ...
    case DataType.TIME_DIFFERENCE:
      lRawData = Buffer.alloc(6);
      if(util.types.isDate(pValue)) {
        // Days since epoch
        const days = Math.floor(pValue.getTime() / 8.64e7);

        // Milliseconds since midnight
        const ms = pValue.getTime() - (days * 8.64e7);

        lRawData.writeUInt32LE(ms, 0);
        lRawData.writeUInt16LE(days, 4);
      }
      break;
  }
...
class DataObject extends EventEmitter {
  ...
  defaultValue?: number | string | Date;
  objFlags?: number;
  _subObjects?: Array<unknown>;
  _raw?: Buffer;

  constructor(index: number, subIndex: number | null, args: DataObjectArgs) {
    super();
    ...
    this.subNumber = args.SubNumber;
    this.objectType = args.ObjectType;
    this.dataType = args.DataType;
    this.lowLimit = args.LowLimit;
    this.highLimit = args.HighLimit;
    this.accessType = args.AccessType;
    this.defaultValue = args.DefaultValue;
    this.pdoMapping = args.PDOMapping;
    this.objFlags = args.ObjFlags;
    this.compactSubObj = args.CompactSubObj;
    ...
    if(this.highLimit !== undefined && this.lowLimit !== undefined) {
      if(this.highLimit < this.lowLimit) { throw RangeError('HighLimit may not be less LowLimit'); }
    }

    // Create raw data buffer
    this._raw = typeToRaw(this.defaultValue ?? 0, this.dataType);
  }
  ...
  set raw(raw) {
    if(raw === undefined || raw === null) { raw = typeToRaw(0, this.dataType); }

    if(Buffer.compare(raw, this.raw) == 0) { return; }
  ...
  _setSubObject(subIndex, args) {
    if(subIndex > this.subNumber) { throw RangeError(`SubIndex must be >= ${this.subNumber}`); }
...
class Eds {
  ...
  getEntry(index) {
    ...
    if(typeof index === 'string') {
      ...
      if(entry && entry.length == 1) { entry = entry[0]; }
    } else {
      // Index lookup
  ...
  removeEntry(index) {
    ...
    if(entry === undefined) { throw ReferenceError(`0x${index.toString(16)} does not exist`); }

    this.nameLookup[entry.parameterName].splice(
      this.nameLookup[entry.parameterName].indexOf(entry), 1);

    if(this.nameLookup[entry.parameterName].length == 0) { delete this.nameLookup[entry.parameterName]; }
  ...
  getSubEntry(index, subIndex) {
    ...
    if(entry.subNumber === undefined) { throw TypeError(`0x${index.toString(16)} does not support sub objects`); }

    if(entry.subNumber < subIndex) { throw ReferenceError(`0x${index.toString(16)}[${subIndex}] does not exist`); }
  ...
  get baudRates() {
    ...
    if(this.deviceInfo.BaudRate_10 == '1') { rates.push(10000); }
    if(this.deviceInfo.BaudRate_20 == '1') { rates.push(20000); }
    if(this.deviceInfo.BaudRate_50 == '1') { rates.push(50000); }
    if(this.deviceInfo.BaudRate_125 == '1') { rates.push(125000); }
    if(this.deviceInfo.BaudRate_250 == '1') { rates.push(250000); }
    if(this.deviceInfo.BaudRate_500 == '1') { rates.push(500000); }
    if(this.deviceInfo.BaudRate_800 == '1') { rates.push(800000); }
    if(this.deviceInfo.BaudRate_1000 == '1') { rates.push(1000000); }
  ...
  _write(fd, data) {
    ...
    if(data.length > 0) { fs.writeSync(fd, data + EOL); }
  }
  ...
  _writeObjects(fd, objects) {
    for(const [key, value] of Object.entries(objects)) {
      if(key == 'SupportedObjects') { continue; }
